...
export function mapMainPageData(raw: MainPageRaw): MainPage {
  return {
    hero: {
      ...
      bullets:
        raw.hero?.bullets?.flatMap((b) =>
          b ? [{ id: b.id, text: b.text ?? null }] : [],
        ) ?? [],
    ...
    tasks: {
      ...
      servicePages:
        raw.tasks?.servicePages_connection?.nodes.map((node) => ({
          id: node.documentId,
    ...
    classesFormat: {
      ...
      items:
        raw.classesFormat?.items?.flatMap((item) =>
          item
            ? [
                {
                  id: item.id,
    ...
    faq: {
      ...
      faqItems:
        raw.faq?.faqItems_connection?.nodes.map((node) => ({
          id: node.documentId,
